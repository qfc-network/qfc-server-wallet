...
- Live audit anchor cron (deferred from M2; M2 P2 shipped only the type shape — actual cron job + on-chain commit lands here, needs `qfc-core` dep and a funded operator account). **v1.2 update:** the M3 skeleton PR shipped `LocalFileAnchor` (file-backed signed JSONL submitter) per [m3-decisions D28](m3-decisions.md#d28); the on-chain submitter remains blocked on `qfc-core` workspace integration per retro-m1-m2 [§3.6](retro-m1-m2.md). **v1.3 status:** unchanged — `LocalFileAnchor` ships; on-chain submitter remains blocked on `qfc-core` integration (retro-v1.3 [§6.2](retro-v1.3.md)). **post-v0.1.0 — shipped.** `ChainAnchor` lands the on-chain submitter **without** a `qfc-core` workspace dep: qfc-core exposes tx submission via its EVM-compatible JSON-RPC (`eth_sendRawTransaction`, same surface qfc-cli/SDKs use), so `ChainAnchor` speaks plain Ethereum JSON-RPC — it builds a legacy EIP-155 tx carrying the daily commitment (`b"qfc-audit-anchor-v1\0" ‖ chain_head[32] ‖ event_count_be[8] ‖ date`) in calldata, signs with a secp256k1 operator key, and broadcasts. Built entirely from existing workspace deps (`k256` + `sha3` + `reqwest`; hand-rolled RLP pinned to the canonical EIP-155 vector) — no alloy/ethers, no `qfc-core` import. Wired into the binary behind `QFC_ANCHOR_RPC_URL` (off by default). The original "needs a `qfc-core` dep" framing was wrong: the EVM-RPC path removes that dependency entirely. Remaining: a funded operator account + a live chain for end-to-end exercise (unit + wiremock integration tests cover construction/signing/submit offline). See [anchor-onchain-decisions.md](anchor-onchain-decisions.md).
